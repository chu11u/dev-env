// Standalone blog post seeder - run inside backend container: node seed_blog.js
...
const posts = [
   {
     ...
     excerpt: "לא בטוחה מה סוג העור שלך? למדי איך לזהות את סוג העור שלך ולבחור את המוצרים הנכונים.",
...
(async () => {
  ...
  let authorId;
  if (!author) {
     const a = await db.author.create({ data: { name: "שרית אלקיים", email: "sarit@saritelkayam.com" } });
     authorId = a.id;
   } else {
     authorId = author.id;
   }
  ...
  for (const post of posts) {
     await db.post.upsert({
       ...
       update: { title: post.title, excerpt: post.excerpt, content: post.content, featuredImage: post.featuredImage, status: post.status, publishedAt: new Date(post.publishedAt), authors: { set: [{ id: authorId }] } },
       create: { ...post, publishedAt: new Date(post.publishedAt), authors: { connect: { id: authorId } } },
      });
