...
const DinnerDetail = ({
  ...
  // Get dishes assigned to this dinner via dinnerDishes
  const dinnerDishLinks = dinnerDishes.filter((dd) => dd.dinnerId === dinnerId);
  const dinnerDishesDetail = dinnerDishLinks.map((dd) => {
    const dish = dishes.find((d) => d.id === dd.dishId);
    const family = families.find((f) => f.id === dd.familyId);
    return { ...dd, dish, family };
  });
  ...
  const updateGuestAttendees = async (guest, attendees) => {
    const updated = {
      ...dinner,
      guestList: dinner.guestList.map((g) =>
        g.familyId === guest.familyId ? { ...g, attendees } : g,
      ),
    };
  ...
  const dinnerDate = dinner.date
    ? new Date(`${dinner.date}T${dinner.time || "19:00"}`)
    : null;
  const isPast = dinnerDate && dinnerDate < new Date();

  // Get total attendees
  const totalAttendees = (dinner.guestList || []).reduce(
    ...
    <div className="container">
      ...
      <div className="card" style={{ textAlign: "center", padding: 32 }}>
        <h1 style={{ fontSize: 28, marginBottom: 8 }}>
          {dinner.name || "ארוחה"}
          {isPast && (
            <span style={{ fontSize: 16, color: "var(--color-text-light)" }}>
              (חלפה)
            </span>
          )}
        </h1>
        ...
        {dinner.notes && (
          <p style={{ marginTop: 16, color: "var(--color-text-light)" }}>
            {dinner.notes}
          </p>
        )}
      </div>
      ...
      <div className="card">
        <h2 style={{ marginBottom: 16 }}>
          👨‍👩‍👧‍👦 מוזמנים ({totalAttendees} אנשים)
        </h2>
        {!dinner.guestList || dinner.guestList.length === 0 ? (
          <p style={{ color: "var(--color-text-light)" }}>
            אין עדיין משפחות מוזמנות. נוסיף דרך דף הארוחות.
          </p>
        ) : (
          <div>
                ...
                >
                  ...
                  >
                    <button
                      ...
                      onClick={() =>
                        updateGuestAttendees(
                          guest,
                          Math.max(0, (guest.attendees || 0) - 1),
                        )
                      }
                    >
                      −
                    ...
                    <button
                      ...
                      style={{ width: 32, height: 32, padding: 0 }}
                      onClick={() =>
                        updateGuestAttendees(guest, (guest.attendees || 0) + 1)
                      }
                    >
                      +
                    </button>
                    <span
                      style={{ fontSize: 13, color: "var(--color-text-light)" }}
                    >
                      אורחים
                    </span>
                  </div>
      ...
      <div className="card">
        ...
        {dinnerDishesDetail.length === 0 ? (
          ...
          <div className="grid-2">
            {dinnerDishesDetail.map(({ dish, family, id: ddId }) => {
              if (!dish) return null;
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 4 }}>
                      ...
                      <button
                        onClick={() => handleUnassignDish(ddId)}
                        style={{
                          background: "none",
                          border: "none",
                          cursor: "pointer",
              ...
              .map((post, idx) => (
                <div
                  ...
                  style={{
                    ...
                    borderBottom:
                      idx <
                      posts.filter((p) => p.dinnerId === dinnerId).length - 1
