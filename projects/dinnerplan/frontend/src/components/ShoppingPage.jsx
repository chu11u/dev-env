...
const ShoppingPage = ({ shopping, setShopping, dishes }) => {
  ...
  const [form, setForm] = useState({ name: '', quantity: '', purchaser: '', purchased: false });
  ...
  const autoGenerateShopping = () => {
    const existingItems = shopping.map(s => s.name.toLowerCase());
    const newItems = [];
    dishes.forEach(dish => {
      if (dish.ingredientList) {
        const ingredients = dish.ingredientList.split(',').map(i => i.trim()).filter(Boolean);
        ingredients.forEach(ingredient => {
          if (!existingItems.includes(ingredient.toLowerCase()) && !shopping.find(s => s.name.toLowerCase() === ingredient.toLowerCase())) {
            newItems.push({
              name: ingredient,
              quantity: '',
          ...
          {filteredShopping.map(item => (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                {item.quantity && <span style={{ color: 'var(--color-text-light)', marginRight: 8 }}>({item.quantity})</span>}
              </div>
              {item.purchaser && (
                <span className="badge badge-accent">👤 {item.purchaser}</span>
              )}
              <button className="btn btn-danger btn-sm" onClick={() => handleDelete(item.id)} style={{ padding: '4px 8px', fontSize: 12 }}>✕</button>
            </div>
          ))}
        </div>
      )}
